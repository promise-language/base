...
**Measurements carry the type they were measured in.** A bare `5` is an int and a float alike, and JSON cannot tell them apart — so a reader could not reconstruct what a gate wrote without the manifest open beside it, and a value disagreeing with what the manifest declared would be a widening nobody could see rather than a mismatch someone can name. Bare numbers are what the cheaper encoding buys, and the envelope's readability is what it pays: an encoding written out by hand emits a measurement as a number and nothing else, which is shorter to write once and unreadable from then on, because an envelope is read by more things than write it and a type dropped at the boundary cannot be recovered from the wire. Carrying the type is what makes the envelope mean the same thing to a reader that it meant to the gate that wrote it. Integers stay integers: a test count encoded as a float is a count nobody can compare exactly. `bool` is the one type JSON describes on its own — `true` is not a number in any reading — and it is declared anyway, so that every measurement is checked against the manifest by the same rule rather than one type being trusted because the wire happened to be unambiguous about it.
...
A member carries a `name` — the thing as the subject names it — an `outcome`, a `detail` when a bad outcome needs explaining, and an `elapsed` when the subject timed it. A member with no name names nothing and is refused, and so is one with no outcome, which is the one thing a member is for; a `detail` and an `elapsed` are each carried in order to say something, so an empty detail and a negative elapsed are refused for the reason a blank `incomplete_reason` is. Each of these is **refused rather than fatal**: an envelope printed by another language that omits a required field is read far enough to be named and turned down, which is the difference between a runner reporting that a gate broke the contract and a runner that is no longer running.
