...
| **Gate output envelope** | a gate and its runner | one gate run's result, as JSON on stdout — measurements, each carrying the type it was measured in, and the reason it measured less than usual if it did; never a verdict, and never whether the run finished, which malformed JSON already says |
